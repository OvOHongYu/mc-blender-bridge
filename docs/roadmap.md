...
> 这个次序 → Z-Fighting。
>
> 修法：注入条件只看资产包的 `use_model`（= "模型不是简单整立方体"），并按
> `AssetPack.cube_overlay_extras()` 把它拆成「整立方体基底 + 叠加层」——
> **基底继续走贪心合并**（草顶是最高频的面），**只单独注入叠加层**并在渲染期沿
> 面法向推出 5/256 方块（`mesher._OVERLAY_PUSH`）。推出刻意留在渲染期、不写进
> 资产包：资产包保持原版模型几何的精确值，共面与否是 Blender 特有的渲染补偿。
> 实测代价：草密集区块 1026 -> 1093 面（+6.5%），且贪心面数完全不变（314），
> 即合并一点没损失；若整模型注入则为 1701 面。
