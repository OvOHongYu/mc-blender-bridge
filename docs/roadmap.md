...
> `grass_block_side_overlay`，后者带 tintindex）。两者顶点集完全相同 → 共面，
> Blender 里 Z-Fighting，草皮那层可能被基底盖住，于是侧面草皮显示成基底贴图里
> 烘焙的平原绿，与顶部的群系色不一致（与 R8 无关，属长期存在）。
> 修法：烘焙期对"同位置但贴图/染色不同"的面，把后续者沿法向推出 5/256 方块
> （`_dedupe_coincident(..., push=0.32)`，单位 1/16）。
